...
pub fn add_processing_plant(game_state: &mut GameState, tier_index: usize) {
    ...
    let new_plant = ProcessingPlantData {
        ...
        tier_index,
        available_tiers: all_tiers, // Use the `all_tiers` variable defined at the start of the function
...
pub fn check_fabricator_inputs(game_state: &GameState, fabricator: &FabricatorData) -> bool {
    if let Some(tier) = fabricator.available_tiers.get(fabricator.tier_index) {
        for (resource_type, required_amount) in &tier.input_resources {
            if game_state.current_resources.get(resource_type).unwrap_or(&0.0) < &(*required_amount as f32) {
                return false; // Not enough of this input resource
            }
        }
        return true; // All inputs are sufficient
    }
    false // Tier not found
}
...
pub fn fabricator_production_system(game_state: &mut GameState, time_delta_secs: f32) {
    for fab in game_state.fabricators.iter_mut() {
        if let Some(tier) = fab.available_tiers.get(fab.tier_index) {
            ...
            let has_inputs = check_fabricator_inputs(game_state, fab);
...
fn generate_unique_id() -> String {
    format!("struct_{}", rand::random::<u32>())
}
...
pub fn add_habitation_structure(game_state: &mut GameState, tier_index: usize) {
    ...
    let new_structure = HabitationStructure {
        ...
        available_tiers: all_tiers, // All possible tiers are available for upgrade
...
pub fn remove_habitation_structure(game_state: &mut GameState, structure_id: &str) {
    let mut inhabitants_to_reassign = 0;
    let mut specialists_to_unassign = 0;
    ...
    if let Some(index) = game_state.habitation_structures.iter().position(|s| s.id == structure_id) {
        ...
        inhabitants_to_reassign = removed_structure.current_inhabitants;
        specialists_to_unassign = removed_structure.assigned_specialists;
    ...
    // Adjust total inhabitants and specialists
    // This is a simplified adjustment. A more complex system might have homelessness or unemployment.
    game_state.total_inhabitants = game_state.total_inhabitants.saturating_sub(inhabitants_to_reassign);
    game_state.assigned_specialists_total = game_state.assigned_specialists_total.saturating_sub(specialists_to_unassign);
...
pub fn upgrade_service_building(game_state: &mut GameState, building_id: &str) {
    if let Some(building) = game_state.service_buildings.iter_mut().find(|b| b.id == building_id) {
        if building.current_tier_index < building.available_tiers.len() - 1 {
            ...
            let next_tier_info = &building.available_tiers[next_tier_index];

            // Assume upgrade cost is the construction cost of the next tier
            let upgrade_cost = next_tier_info.construction_credits_cost;

            if game_state.credits < upgrade_cost as f64 {
                println!("Not enough credits to upgrade Service Building {} to {}. Required: {}, Available: {:.2}", building_id, next_tier_info.name, upgrade_cost, game_state.credits);
                return;
            }
            game_state.credits -= upgrade_cost as f64;
            println!("Upgraded Service Building {} to {} for {} credits. Remaining credits: {:.2}", building_id, next_tier_info.name, upgrade_cost, game_state.credits);

            building.current_tier_index = next_tier_index;
            if building.assigned_specialists > next_tier_info.specialist_requirement {
                let to_unassign = building.assigned_specialists - next_tier_info.specialist_requirement;
                building.assigned_specialists -= to_unassign;
                game_state.assigned_specialists_total -= to_unassign;
            }
            update_civic_index(game_state);
            println!("Upgraded Service Building {} to {}", building_id, next_tier_info.name);
        } else {
...
fn update_colony_stats_system(
    ...
    stats.total_housing = game_state.available_housing_capacity; // Use new GameState field
    ...
    stats.total_jobs = game_state.total_specialist_slots; // Now reflects slots from habitation AND zones
